...
public class OAuth20GoogleServiceImpl implements OAuth20GoogleService {
    ...
    @Override
    public Response getResponse(OAuth20Service service, OAuthRequest request, OAuth2AccessToken token) throws InterruptedException, ExecutionException, IOException {
    ...
    public void setAuthentication(String code) throws Exception {
        ...
        Response response = this.getResponse(service, new OAuthRequest(Verb.GET, "https://www.googleapis.com/oauth2/v1/userinfo?alt=json"), token);
        Map<String, String> mapFromJSON = this.convertJSONtoMap(response.getBody());
        SecurityContextHolder.getContext().setAuthentication(new UsernamePasswordAuthenticationToken(mapFromJSON.get("name"),
                "",
